...
def pytest_collection_modify_items(config, items):
    """Automatically add markers to tests based on their names or location"""
    # Suppress unused argument warning - config parameter is required by pytest
    _ = config

    for item in items:
        # Add unit marker to unit tests
        if "unit" in item.name or "test_unit" in item.name:
